<?php
    ...
    while($a<7)
    {
        ...
        echo '<tr>';
        echo '<td>'.$a.'</td>';
        echo '<td><a href="#" OppId="'.$Opp.'" class="btn btn-success myFunction">'.$OppName.'</a></td>';
        echo '<td> '.$result.' </td>';
        echo '</tr>';
    ...
    if($Week>6)
    {
        $Opponent = "PlayoffWk1Opp";

        $Opp = $row[$Opponent];
        if($Opp != NULL){$OppName = ReadScalar(ExecuteSqlQuery("SELECT TeamName FROM fantteams WHERE TeamId = '$Opp'"));}else{$OppName="No Playoffs";}
        if($Week != $a && $Opp != NULL){$OppScore = ReadScalar(ExecuteSqlQuery("SELECT Score FROM fantresult WHERE Team = '$Opp' && Week = '$a'"));}
        else if($Opp != NULL){
            $TInfo =  ReadScalar(ExecuteSqlQuery("SELECT * FROM fantteamsstarters WHERE LeagueId = '$LeagueId' && TeamId = '$Opp'"));
    ...
    $(".myFunction").on('click', function(){			
        {
            ...
            $("#TeamInner").show();
            $.post("StartingLineupViewing.php", {LeagueId:LeagueId, OppId:OppId, TeamId:TeamId}, function(data){
                $("#TeamInner").html(data);
            });
        }
    });
