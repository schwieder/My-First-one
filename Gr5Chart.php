<?php
	...
	<tr>'
		...
		<th><input type='button' rel="EarningsCurr" id=<?php echo $StockType;?> align="right" style="height:20px;width:150px;" class='btn btn-success Chart' value='Current Earnings'></th>
		<th><input type='button' rel="EarningsPrev" id=<?php echo $StockType;?> align="right" style="height:20px;width:150px;" class='btn btn-success Chart' value='Previous Earnings'></th>
		<th><input type='button' rel="Shares" id=<?php echo $StockType;?> align="right" style="height:20px;width:150px;" class='btn btn-success Chart' value='# of Shareholders'></th>
		<?php
	echo '
		';
	echo '</tr><tbody>';

	foreach(MysqlFetchData(ExecuteSqlQuery("SELECT * FROM stocks5 WHERE Sector = '$StockType' ORDER BY $Sort")) as $row)
	{
		echo '<tr>';
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".Buy").on('click', function(){
			...
			$.post("Gr5Buy.php", {StockId: StockId}, function(data){
				$("#Chart").html(data);
			});
		});
