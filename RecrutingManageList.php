...
$TeamNo = ReadScalar(ExecuteSqlQuery("SELECT teamNumber FROM hkyteams WHERE teamId ='$TeamId' AND ownerId = $UserId"));
if($TeamNo == "1"){$List = "team1List"; $Call = "team1Call";}
else if($TeamNo == "2"){$List = "team2List"; $Call = "team2Call";}
else if($TeamNo == "3"){$List = "team3List"; $Call = "team3Call";}
else if($TeamNo == "4"){$List = "team4List"; $Call = "team4Call";}
else if($TeamNo == "5"){$List = "team5List"; $Call = "team5Call";}
else if($TeamNo == "6"){$List = "team6List"; $Call = "team6Call";}
...
echo '</tr><tbody class="gridtable">';
    ...
    foreach(MysqlFetchData(ExecuteSqlQuery("SELECT * FROM recruits WHERE leagueId = ".$LeagueId." AND ".$List." = 'Y'")) as $row)
    {
        ...
        if($Call == 'Y'){
            echo "<td>Already Called</td>";
        }
        else{
            echo '<td><input type="button" name="Prestige" id="'.$TeamNo.'" idR="'.$RecruitId.'" class="btn btn-success Prestige" value="Call"></td>';
            echo '<td><input type="button" name="Facilities" id="'.$TeamNo.'" idR="'.$RecruitId.'" class="btn btn-success Facilities" value="Call"></td>';
            echo '<td><input type="button" name="Academics" id="'.$TeamNo.'" idR="'.$RecruitId.'" class="btn btn-success Academics" value="Call"></td>';
            echo '<td><input type="button" name="Depth" id="'.$TeamNo.'" idR="'.$RecruitId.'" class="btn btn-success Depth" value="Call"></td>';
        }
...
<script>
     function Pos() {
        ...
        $.post("RecruitManageListSortByPos.php", {TeamId : TeamId, LeagueId: LeagueId}, function(data){
            $("#Team2").html(data);
        });
     }
    $(".Remove").on('click', function(){			
        {
            ...
            var LeagueId = <?php echo $LeagueId; ?>;
            var YN = "N";
            $.post("RecruitManageListAddRemove.php", {YN : YN, recruitId : recruitId, teamNo : teamNo, LeagueId: LeagueId, TeamId: TeamId}, function(data){
                $("#Team2").html(data);
            });
        }
    });
    $(".Prestige").on('click', function(){			
        {
            ...
            var LeagueId = <?php echo $LeagueId; ?>;
            var Pitch = "prestige";
            $.post("RecruitManageListPitch.php", {Pitch : Pitch, recruitId : recruitId, teamNo : teamNo, LeagueId: LeagueId, TeamId: TeamId}, function(data){
                $("#Team2").html(data);
            });
        }
    });
