<?php
	...
	$LastUpdate = ReadScalar(ExecuteSqlQuery("SELECT LastUpdate5 FROM clock WHERE Id='1'"));
	...
	if($NextAdvance <= $today)
	{
		...
		$Count = ReadScalar(ExecuteSqlQuery("SELECT COUNT(StockId) FROM stocks5"));
		echo $Count;
		$x = 1;
		While($x<=$Count)
		{
			$Stock = ReadScalar(ExecuteSqlQuery("SELECT * FROM stocks5 WHERE StockId ='$x'"));
			...
			$PrevPrice = ($EarningsPrev/$Shares)+$Brand;
			$EarningsCurr = $EarningsPrev/$Growth;
			$PrevPrice = number_format((float)$PrevPrice, 2, '.', '');
			$EarningsCurr = ceil($EarningsCurr);
			
			$insertQuery = "UPDATE stocks5 SET EarningsPrev=?, EarningsCurr=?, PrevPrice=? WHERE StockId=?";
		...
		$today2 = date('Y-m-d');
		$insertQuery = "UPDATE clock SET LastUpdate5=? WHERE Id=?";
